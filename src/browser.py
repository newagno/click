...
class IncryptedBrowser:
    ...
    def execute_claim(self) -> str:
        ...
        # On Linux (GitHub Actions) we run under xvfb-run (headless=False)
        headless_mode = False
        ...
        with SB(uc=True, headless=headless_mode, proxy=self.proxy, binary_location=binary_location) as sb:
            ...
            try:
                ...
                print(f"DEBUG: Connection test failed! Error: {e}")

            # ── STEP 1: Open the account page directly ─────────────────────
            # Direct access to the account page is much more stable than clicking popups on the homepage.
            ...
            found = False
            for i in range(15):
                # If Turnstile pops up during page load, try to bypass it
                if sb.is_element_visible('iframe[src*="turnstile"]'):
            ...
            try:
                sb.wait_for_element(slider_selector, timeout=10)
